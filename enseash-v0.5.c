    ...
    while (1) {
        ...
        if (!(strcmp(inbuff, "exit"))) {
            ...
            if (pid != 0){
                ...
                if(snprintf(output_buffer, "exit:%d|%ldms", status, stop_ms-start_ms) > OUTPUT_BUFFER_SIZE){
                    write(STDOUT, "error buffer overflow", 20);
                }
                else {
                    sprintf(output_buffer, "exit:%d|%ldms", status, stop_ms-start_ms);
                }
